$(document).ready(function() {
	...
	$('.vitrine li').each(function(){
		if (($(this).find('.oldPrice').length == 0)&&($(this).find('.ouAte').length != 0)){
			$(this).find('.newPrice').prepend('à vista ');
...
// Barra suspensa superior 
	...
	if ($('#discount .flag').length != 0 ){
		...
		if (nd[1] == " desconto Inteligente"){
			$('#discontoBoleto').html('<span class="discontoBoleto">R$'+vcd+'</span> à vista no <strong>boleto</strong>');
...
$(document).ajaxStop(function(){
	...
	$('.vitrine li').each(function(){
		...
		if ($(this).find('em.installment').filter(":contains(ou até)").length != 0){
...
//função para verificar se há produto em estoque
...
$.fn.quickBuySku = function(){
	...
	//função para mudar o href para o title sku
			...
			try{
				...
				$.ajax({
					...
					success: function() {
						...
						/*}else{
							alert('Não há quantidade suficiente em estoque deste produto');
						}*/
					}
				});
